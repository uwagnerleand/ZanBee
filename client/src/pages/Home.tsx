...
import { Search, ShoppingBag, Menu, X, ArrowRight, ChevronRight, Heart, MessageCircle, Minus, Plus, Trash2, Check, Instagram, MapPin } from "lucide-react";
...
type Product = { id:number; name:string; category:string; price:number; oldPrice?:number; sizes:string[]; colors:string[]; image:string; description:string; badge?:string };
const products: Product[] = [
  { id:1, name:"Conjunto Infantil Floresta", category:"Meninos", price:40.0, sizes:["6 a 7 anos"], colors:["Verde","Marrom"], image:FLORESTA, description:"Conjunto infantil da coleção Universo Encantado, com estampa da Turma da Floresta e bermuda confortável.", badge:"Universo Encantado" },
  { id:2, name:"Conjunto Infantil Homem-Aranha", category:"Meninos", price:40.0, sizes:["6 a 7 anos"], colors:["Vermelho","Azul"], image:ARANHA, description:"Conjunto infantil temático Homem-Aranha da coleção Universo Encantado, confortável e cheio de estilo.", badge:"Universo Encantado" },
  { id:3, name:"Conjunto Infantil Gamer Loading", category:"Meninos", price:40.0, sizes:["6 a 7 anos"], colors:["Azul","Vermelho"], image:GAMER, description:"Conjunto infantil gamer da coleção Universo Encantado, com camiseta estilosa e bermuda leve.", badge:"Universo Encantado" },
  { id:4, name:"Conjunto Infantil Urso Moderno", category:"Meninos", price:40.0, sizes:["6 a 7 anos"], colors:["Azul","Laranja"], image:URSO, description:"Conjunto infantil descolado da coleção Universo Encantado, com estampa de urso e bermuda vibrante.", badge:"Universo Encantado" },
  { id:5, name:"Conjunto Infantil Dinossauro Explorer", category:"Meninos", price:40.0, sizes:["6 a 7 anos"], colors:["Preto","Azul"], image:DINOSSAURO, description:"Conjunto infantil temático de dinossauro da coleção Universo Encantado, com camiseta preta estampada e bermuda azul.", badge:"Universo Encantado" }
];
const categories = [
...
export default function Home() {
  ...
  return <div className="min-h-screen bg-[#fffaf2] text-[#4b2b1d] selection:bg-[#f5c84b]/40">
    ...
    <header className="sticky top-0 z-40 border-b border-[#eadbc7] bg-[#fffaf2]/95 backdrop-blur-md"><div className="container flex h-[76px] items-center justify-between gap-5">
      ...
      <nav className="hidden xl:flex items-center gap-6 text-[13px] font-semibold text-[#6d4b39]"><button onClick={()=>scrollTo("inicio")}>Início</button><button onClick={()=>scrollTo("produtos")}>Produtos</button><button onClick={()=>scrollTo("contato")}>Contato</button></nav>
    ...
    </div>{menuOpen&&<div className="mobile-menu xl:hidden">{["Início","Produtos","Contato"].map(item=><button key={item} onClick={()=>scrollTo(item==="Início"?"inicio":item==="Contato"?"contato":"produtos")}>{item}<ChevronRight size={15}/></button>)}</div>}</header>
    <main id="inicio">
      ...
      <section className="catalog-section" id="produtos"><div className="container"><div className="section-heading"><div><span className="eyebrow">Vitrine ZanBee</span><h2>Coleção Universo Encantado (6 a 7 anos)</h2></div></div><div className="catalog-toolbar"><div className="search-large"><Search size={18}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Pesquisar produtos..." /></div></div><div className="product-grid">{filtered.map(p=><article className="product-card" key={p.id}><div className="product-image"><SafeImage src={p.image} alt={p.name}/>{p.badge&&<span className="badge">{p.badge}</span>}<button className="heart-btn" aria-label="Favoritar"><Heart size={17}/></button></div><div className="product-info"><span className="product-category">{p.category}</span><h3>{p.name}</h3><div className="price-row"><strong>{money(p.price)}</strong>{p.oldPrice&&<del>{money(p.oldPrice)}</del>}</div><p>Tamanhos: {p.sizes.join(", ")}</p><div className="product-actions"><button className="detail-btn" onClick={()=>setSelected(p)}>Ver detalhes</button><button className="add-btn" onClick={()=>add(p)}>Adicionar <Plus size={15}/></button></div></div></article>)}</div>{!filtered.length&&<div className="empty-state">Nenhum produto encontrado. Tente outra busca.</div>}</div></section>
    ...
    <footer><div className="container footer-main"><div className="footer-brand"><img src={LOGO} alt="ZanBee"/><p>O lado doce de vestir.</p><span>Óbidos – Pará</span></div><div><h4>Navegue</h4><button onClick={()=>scrollTo("inicio")}>Início</button><button onClick={()=>scrollTo("produtos")}>Produtos</button><button onClick={()=>scrollTo("contato")}>Contato</button></div><div><h4>Fale com a gente</h4><a href={WA} target="_blank" rel="noreferrer">(93) 99157-4982</a><a href="https://wa.me/5593991589475" target="_blank" rel="noreferrer">(93) 99158-9475</a><a href="https://instagram.com/zanbeeobidos" target="_blank" rel="noreferrer">@zanbeeobidos</a></div><div className="footer-note"><img src={BEE} alt=""/><p>Moda infantil escolhida com carinho para as famílias de Óbidos.</p></div></div><div className="container footer-bottom"><span>© 2026 ZanBee. Dados de produtos demonstrativos.</span><span>Roupas • Calçados • Acessórios</span></div></footer>
